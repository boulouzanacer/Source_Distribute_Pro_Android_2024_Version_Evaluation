...
public class ActivityAchat extends AppCompatActivity implements RecyclerAdapterCheckProducts.ItemClick {
                ...
                new SweetAlertDialog(ActivityAchat.this, SweetAlertDialog.WARNING_TYPE)
                        ...
                        .setConfirmClickListener(sDialog -> {
                            ...
                            try {
                                ...
                                controller.update_pamp_on_delete_produit(final_panier.get(info.position).codebarre,
                                        final_panier.get(info.position).stock_produit,
                                        final_panier.get(info.position).pa_ht_produit,
                                        final_panier.get(info.position).qte,
                                        final_panier.get(info.position).pa_ht,
                                        final_panier.get(info.position).pa_ht);

                                initData();
                                //PanierAdapter.RefrechPanier(final_panier);
                                PanierAdapter = new ListViewAdapterPanierAchat(ActivityAchat.this, R.layout.transfert2_items, final_panier, TYPE_ACTIVITY);
                                expandableListView.setAdapter(PanierAdapter);
    ...
    public void onClick(View v, int position, PostData_Produit item) throws ParseException {
        ...
        achat2.produit = item.produit;
        achat2.codebarre = item.code_barre;
        achat2.stock_produit = item.stock;
        achat2.tva = item.tva;
        achat2.colissage = item.colissage;
        achat2.pa_ht = item.pa_ht;
        achat2.pa_ht_produit = item.pa_ht;
    ...
    private void selectProductFromScan(String resultscan) throws ParseException {
        ...
        if (produits.size() == 1) {
            ...
            achat2.codebarre = produits.get(0).code_barre;
            achat2.stock_produit = produits.get(0).stock;
            achat2.tva = produits.get(0).tva;
            achat2.colissage = produits.get(0).colissage;
            achat2.pa_ht = produits.get(0).pa_ht;
            achat2.pa_ht_produit = produits.get(0).pa_ht;
